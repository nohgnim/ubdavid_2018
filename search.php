<?php
   ...
   class Page
   {
      ...
      function search ($string)
      {
         ...
         preg_match_all("/\b([^\s]+)\b/",trim($string), $word_array_2);
         ...
         foreach ($word_array[1] as $key => $word)
         {
            ...
            else
            {
               ...
               $this->bolded_content = preg_replace("/\b(" . preg_quote($word, '/') . ")\b/i",
                                                 '<strong>\1</strong>',
                                                 $this->bolded_content);
            }
      ...
      {
         preg_match_all("/\b" . preg_quote($needle, '/') . "\b/i", $haystack, $matches);
   ...
   else if(trim($query) != '')
   {
      ...
      {
         $title = 'Search results for &quot;' . htmlentities($query, ENT_QUOTES, 'UTF-8') . '&quot;';
         ...
         $q = $query;
         $query = htmlentities(urldecode($query), ENT_QUOTES, 'UTF-8');
         ...
         if ($cnt > 0)
         {
            ...
            if ($start < $cnt)
            {
               ...
               // Check to see if we need to add navigational links to our search
         	    if ($cnt > $prefs->resultsperpage)
         	    {
         	       ...
         	       for ($i = $page - 4; $i != $page; $i++)
         	       {
         	          ...
         	          {
         	             $navigation .= "    <a href=\"" .
         	                            htmlentities(preg_replace('/&p=\d*/','',$_SERVER['REQUEST_URI']), ENT_QUOTES, 'UTF-8') .
         	                            "&amp;p=$i\">$i</a>&nbsp;\n";
         	          }
         	       }
               ...
               $result_info = "<p id=\"result_info\">Found $cnt results for &quot;" .
                              (($prefs->htmldef == 'true') ?
                              (preg_replace_callback("/\b([^\s]+)\b/", "fix_entities", $q))
                              :
                              htmlentities($q, ENT_QUOTES, 'UTF-8')) .
                              "&quot; in $et " .
